# ----------------------------------------------------------------------------
# Original file: hdi2023_analysis_Geography_HDI.R
# Geography of HDI (2023 data): evolution of 01_hdi2022_regions.R.
# Adds I/C and W/H structural ratios and the benchmark profiles (pp)
# used by scripts 04 and 05.
#
# Notes / known issues (code kept as in the original):
#  - expects data/HDI2023.txt (not distributed in this repo)
#  - defines `regions` and `pp` used by 04 and 05 - run this first
#  - the sensitivity table (dfSensitivity, 'Tabla Sensibilidades') is filled in manually from the printed output of the loop above it
# ----------------------------------------------------------------------------
...
poset_data <- read_delim("data/HDI2023.txt", delim = "\t")
...
for(i in 1:length(regions)) {
  hdi_regions_filtered <- read_delim("data/HDI2023.txt", delim = "\t",show_col_types = FALSE)
...
#### POSET amb altures mitjana ####
  ...
  # png(file=paste("output/POSet_HDI23_avgHeight_", regions[i], ".png", sep = ""), width=1100, height=860)
  ...
  ggsave(filename = paste("output/Sensitivity_HDI23_", regions[i], ".png", sep = ""), 
...
png(file="output/3-POSET_HDI23_Embedded.png", res = 240, height = 3600, width = 4200)
par(mfrow = c(3,2))
par(mar = c(0,0, 0, 0))
for(i in 1:length(regions)) {
  hdi_regions_filtered <- read_delim("data/HDI2023.txt", delim = "\t")
...
png(file="output/4-POSET_HDI23_Embedded_AverageHeights.png", res = 240, height = 4200, width = 2700)
par(mfrow = c(3,2))
for(i in 1:length(regions)) {
  hdi_regions_filtered <- read_delim("data/HDI2023.txt", delim = "\t")
...
hdi_regions <- read_delim("data/HDI2023.txt", delim = "\t")
