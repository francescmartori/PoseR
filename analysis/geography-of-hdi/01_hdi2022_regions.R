# ----------------------------------------------------------------------------
# Original file: hdi_regions.R
# Geography of HDI (2022 data): global and per-region posets, average
# heights, sensitivity analysis, dominance/incomparability, k-means
# comparison and benchmark (embedded scale) profiles.
#
# Notes / known issues (code kept as in the original):
#  - expects data/HDI2022.txt (not distributed in this repo)
#  - figures are written to output/ (git-ignored)
# ----------------------------------------------------------------------------
...
for(i in 1:length(regions)) {
  hdi_regions_filtered <- read_delim("data/HDI2022.txt", delim = "\t")
  ...
  png(file=paste("output/POSet_HDI22_", regions[i], "_byDevRegion.png", sep = ""), width=1100, height=860)
  ...
  png(file=paste("output/POSet_HDI22_avgHeight_", regions[i], ".png", sep = ""), width=1100, height=860)
  ...
  ggplot(dfSensitivity, aes(columns, sens)) + geom_bar(stat = "identity") + 
   ...
   ggsave(filename = paste("output/Sensitivity_HDI22_", regions[i], ".png", sep = ""), 
...
for(i in 1:length(regions)) {
  hdi_regions_filtered <- read_delim("data/HDI2022.txt", delim = "\t",show_col_types = FALSE)
...
for(i in 1:length(regions)) {
  ...
  png(file=paste("output/POSet_HDI22_", regions[i], "_EmbeddedScale.png", sep = ""), width=1100, height=860)
  ...
  png(file=paste("output/POSet_HDI22_", regions[i], "_EmbeddedScale_Avgheights.png", sep = ""), width=1100, height=860)
