...
plot_avgheight_poset <- function(Z.cov, Z, V, method, color, title, input) {
  # Compute multiple ranking profiles for poset layout
  M <- MRP(Z, method = method)
  avg_heights <- colSums(M)  # Sum ranks to get average height per node
...
plot_standard_poset <- function(Z.cov, V, color = "white", title = NULL, input = "min", chain = NULL) {
  # Define plot dimensions with some padding
  xrange <- range(V$x)
  yrange <- range(V$y)
  xpad <- diff(xrange) * 0.15
  ypad <- diff(yrange) * 0.15
  ...
  # Get list of cover relations (edges)
  edge_list <- which(Z.cov == 1, arr.ind = TRUE)
  ...
  for (i in seq_len(nrow(edge_list))) {
    ...
    # Draw edge with color depending on chain membership
...
poset_stats <- function(Z) {
  ...
  print(relations) #does the same thing as calling summary(poset_from_incidence(Z))[3]
